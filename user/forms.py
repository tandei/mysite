...
class ChageAliasNameForm(forms.Form):
    aliasname_new = forms.CharField(label='新的昵称', 
        max_length=20,
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ChageAliasNameForm, self).__init__(*args, **kwargs)
    ...
    def clean_aliasname_new(self):
        aliasname_new = self.cleaned_data.get('aliasname_new', '').strip()
        if aliasname_new == '':
            raise forms.ValidationError('新的昵称不能为空')
        return aliasname_new
...
class BindEmailForm(forms.Form):
    ...
    def clean_email(self):
        email = self.cleaned_data['email']
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError('该邮箱已被绑定,不可重复绑定')
        return email
